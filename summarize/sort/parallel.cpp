...
void sortBy(void(*op)(int* , int* ),int* copyTestArray, int totalNum, string opName)
{
    ...
    cout<<(t_end-t_start)*1000/CLOCKS_PER_SEC<<endl;
}
...
int main()
{
    ...
    */
    ...
    {
        int* cTArray=new int[sFArray];//sizeForArray, copyToArray
        memcpy(cTArray,tArray,sFArray*sizeof(cTArray[0]));
    ...
    auto HeapSortTest=[](int* tArray,int sFArray)
    {
        int* cTArray=new int[sFArray];
        memcpy(cTArray,tArray,sFArray*sizeof(cTArray[0]));
        sortBy(HeapSort,cTArray,sFArray,"HeapSort");
        delete[] cTArray;
    };
    ...
    thread4.join();
    thread thread5(HeapSortTest,testArray,totalNum);
    thread5.join();
